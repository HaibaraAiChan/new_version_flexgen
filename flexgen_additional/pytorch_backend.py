...
class TorchDevice:
    ...
    def mha(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        hidden_1 = post_attention_layernorm(inputs.data )
        ...
        # return
        # modified --------------end
        # print("inputs.data.device.type ",inputs.data.device.type)
        # print("w_ln.data.device.type ",w_ln.data.device.type)
        # print("b_ln.data.device.type ",b_ln.data.device.type)
        # print('h ', h)
        
        # hidden = F.layer_norm(inputs.data, (h,), weight=w_ln.data, bias=b_ln.data)
        # print('hidden.shape ', hidden.shape)
        # print("hidden.device.type ",hidden.device.type)
        # return
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        print('inputs.data.shape ', inputs.data.shape)
        value.add_(inputs.data) # Add & Norm # torch tensor.add_() elementwise addition here
        print('value.shape after add_ ', value.shape)
        if donate[0]: inputs.delete()
        if donate[1]: attention_mask.delete()
