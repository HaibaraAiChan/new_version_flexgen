...
class OptLM_MP:
    def __init__(self,
        ...
        mp_size = self.tensor_model_parallel_size

        # cache[j][k]
        self.cache_home = array_3d(num_layers, num_gpu_batches, mp_size, ValueHolder)
        self.cache_read_buf = array_3d(num_layers, num_gpu_batches, mp_size, ValueHolder)
        self.cache_write_buf = array_3d(num_layers, num_gpu_batches, mp_size, ValueHolder)
        # weight[j]
        self.weight_read_buf = array_2d(num_layers, mp_size, ValueHolder)
        # attention_mask[k]
        self.attention_mask = array_2d(num_gpu_batches, mp_size, ValueHolder)
    ...
    def generation_loop_overlap_single_batch(self):
        ...
        for i in range(self.execute_gen_len):
            ...
            if self.task.stop and np.all(self.stopped):
                break
    def generation_loop_overlap_multi_batch_mp(self):
        # Prologue
        for k in range(self.num_gpu_batches):
            for m in range(self.tensor_model_parallel_size):
                self.load_weight(0, 0, k, m)
        for m in range(self.tensor_model_parallel_size):
            self.load_hidden(0, 0, 0, m)
        self.sync()
        ...
        for i in range(self.execute_gen_len):
            ...
            for k in range(self.num_gpu_batches):
                for m in range(self.tensor_model_parallel_size):
                    self.update_attention_mask(i, k, m)
            for j in range(self.num_layers):
                for k in range(self.num_gpu_batches):
                    for m in range(self.tensor_model_parallel_size):
                        self.load_weight(i, j+1, k, m)
                        self.load_cache(i, j, k+1, m)
                        self.store_hidden(i, j, k-1, m)
                        self.load_hidden(i, j, k+1, m)
                        self.compute_layer(i, j, k, m)
                        self.store_cache(i, j, k-1, m)
                        self.sync()
                    self.sync()
            timers("generate").stop()
